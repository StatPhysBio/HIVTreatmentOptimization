...
filepath = "/Users/lamont/Dropbox/Colin_ControlTheory/HIV trapping code/snpanalysis_cladeb.h5"
fid = h5open(filepath, "r+")
dictovec(a::Dict) = [a["$i"] for i in 1:length(a)] #auxiliary function for turning dictionaries into Vector
for ab in fid
    global bayesout = []
    for site in ab
        ...
        for patient in site["snpdata"] ]
            for avg in [0, 1]
                theta_list = Tomoko.baysian_pop_rs(ls; samples = 4*10^3, burn_in = 2*10^2, avg = avg)
                site["bayes_avg$(round(avg,digits=2))"] = theta_list
                println("$avg")
            end
    end
end
